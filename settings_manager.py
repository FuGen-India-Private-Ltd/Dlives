...
DEFAULT_SETTINGS = {
    ...
    "pomodoro_dnd_notifications": True,
    "pomodoro_allow_timetable": True,
    "pomodoro_allow_alarms": True,
...
class SettingsManager(QObject):
    ...
    def __init__(self):
        ...
        self.cleanup_legacy_registry_keys()
    ...
    def load_settings(self) -> dict:
        ...
        try:
            val = int(settings.get("hover_delay_ms", 300))
            settings["hover_delay_ms"] = max(50, min(1000, val))
        except (ValueError, TypeError):
            settings["hover_delay_ms"] = 300
    ...
    def cleanup_legacy_registry_keys(self):
        """Scrub legacy autorun registry keys ('SanLives', 'DynamicIsland', 'DLives', 'DynamicIslandPro') from Windows Run."""
        try:
            with winreg.OpenKey(winreg.HKEY_CURRENT_USER, REG_PATH, 0, winreg.KEY_ALL_ACCESS) as key:
                for legacy_key in ["SanLives", "DynamicIsland", "DLives", "DynamicIslandPro"]:
                    try:
                        winreg.DeleteValue(key, legacy_key)
                        print(f"[REGISTRY MIGRATION]: Cleaned up legacy '{legacy_key}' autostart key.", flush=True)
                    except FileNotFoundError:
                        pass
        except Exception:
            pass
