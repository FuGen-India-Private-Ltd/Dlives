...
def backup_corrupt_file(file_path: str):
    """Safely creates a timestamped .corrupt.bak copy of a damaged file before overwriting defaults."""
    if not file_path or not os.path.exists(file_path):
        return
    try:
        timestamp = int(datetime.now().timestamp())
        bak_path = f"{file_path}.corrupt_{timestamp}.bak"
        shutil.copy2(file_path, bak_path)
        print(f"[STORAGE BACKUP]: Backed up corrupted file '{file_path}' to '{bak_path}'", flush=True)
    except Exception as e:
        print(f"[STORAGE BACKUP WARN]: Failed to back up '{file_path}': {e}", flush=True)

def sanitize_filename(name: str, default: str = "untitled") -> str:
    """Sanitizes user input into a safe, valid Windows filename capped at 64 characters."""
...
def get_app_data_dir() -> str:
    ...
    old_dirs = [
        os.path.join(appdata, "SanLives"),
        os.path.join(appdata, "DynamicIsland"),
        os.path.join(appdata, "DynamicIslandPro"),
        os.path.join(appdata, "DLives")
    ]
    data_dir = os.path.join(appdata, "Dlives")

    # One-time migration check: auto-detect and migrate old folder contents to the new name on first launch
    marker_file = os.path.join(data_dir, ".migration_complete")
    if not os.path.exists(marker_file):
        for old_dir in old_dirs:
            if os.path.exists(old_dir) and old_dir.lower() != data_dir.lower():
                try:
                    if not os.path.exists(data_dir):
                        ...
                        # If target exists, merge only non-overlapping legacy files (never overwrite newer files!)
                    ...
                    print(f"[DATA MIGRATION EXCEPTION]: Failed to migrate data from '{old_dir}': {e}", flush=True)
...
def safe_atomic_write_json(file_path: str, data):
    """Writes JSON data atomically using a temporary file with retry loop, mutex lock & fallback."""
...
def safe_atomic_write_text(file_path: str, text: str):
    """Writes text data atomically using a temporary file with retry loop, mutex lock & fallback."""
    with GLOBAL_STORAGE_LOCK:
        dir_name = os.path.dirname(file_path)
        temp_name = None
        try:
...
class StorageManager(QObject):
    ...
    def __init__(self, parent=None):
        ...
        self._lock = GLOBAL_STORAGE_LOCK
    ...
    def sync_external_note_files(self):
        ...
        if getattr(self, '_watcher_blocked', False):
            return
        ...
        with self._lock:
            ...
            if index_changed:
                if not remaining_index:
                    ...
                    def_path = os.path.join(self.notes_dir, default_note["filename"])
                    def_content = "Welcome to Dlives!\n\n• Quick Notes & Pinning\n• Alarms & Timetable Schedule\n• System Controls & Mini Taskbar\n• File Shelf & Drop Zone"
                    safe_atomic_write_text(def_path, def_content)
                    self._note_cache[default_note["filename"]] = def_content
        ...
        with self._lock:
            ...
            if os.path.exists(self.notes_index_file):
                try:
                    ...
                    backup_corrupt_file(self.notes_index_file)
            ...
            def_path = os.path.join(self.notes_dir, default_note["filename"])
            safe_atomic_write_text(def_path, welcome_text)
            self._note_cache[default_note["filename"]] = welcome_text
    ...
    def delete_note(self, note_id: str):
        with self._lock:
            ...
            if note_to_delete:
                ...
                filename = note_to_delete.get("filename", "")
                filepath = os.path.join(self.notes_dir, filename)
                self._watcher_blocked = True
                try:
                    if os.path.exists(filepath):
                        try:
                            self.notes_watcher.removePath(filepath)
                        except Exception:
                            pass
                        os.remove(filepath)
                except Exception as e:
    ...
    def rename_note(self, note_id: str, new_title: str):
        """Renames note title in index AND physically renames underlying disk file without desync."""
        with self._lock:
            index = self.load_notes_index()
            target_note = next((n for n in index if n.get("id") == note_id), None)
            if not target_note:
                return

            clean_title = (new_title.strip() or "Untitled")[:120]
            old_filename = target_note.get("filename", "")
            old_filepath = os.path.join(self.notes_dir, old_filename) if old_filename else ""

            # Preserve timestamp prefix if available
            prefix = ""
            if old_filename and "_" in old_filename:
                prefix = old_filename.split("_")[0] + "_"
            else:
                prefix = datetime.now().strftime("%Y%m%d_%H%M%S") + "_"

            sanitized = sanitize_filename(clean_title, "Note")
            new_filename = f"{prefix}{sanitized}.txt"
            new_filepath = os.path.join(self.notes_dir, new_filename)

            if old_filepath and os.path.exists(old_filepath) and old_filename != new_filename:
                if os.path.exists(new_filepath) and new_filepath.lower() != old_filepath.lower():
                    import uuid
                    new_filename = f"{prefix}{sanitized}_{uuid.uuid4().hex[:4]}.txt"
                    new_filepath = os.path.join(self.notes_dir, new_filename)

                self._watcher_blocked = True
                try:
                    try:
                        self.notes_watcher.removePath(old_filepath)
                    except Exception:
                        pass
                    os.replace(old_filepath, new_filepath)
                    try:
                        self.notes_watcher.addPath(new_filepath)
                    except Exception:
                        pass

                    if old_filename in self._note_cache:
                        self._note_cache[new_filename] = self._note_cache.pop(old_filename)
                    if old_filename in self._dirty_note_buffers:
                        self._dirty_note_buffers[new_filename] = self._dirty_note_buffers.pop(old_filename)
                except Exception as e:
                    print(f"Error renaming physical note file: {e}")
                finally:
                    self._watcher_blocked = False

                target_note["filename"] = new_filename

            target_note["title"] = clean_title
        ...
        with self._lock:
            if os.path.exists(self.tasks_file):
                try:
                    ...
                    backup_corrupt_file(self.tasks_file)
            return []
        ...
        with self._lock:
            if os.path.exists(self.calendar_file):
                try:
                    ...
                    backup_corrupt_file(self.calendar_file)
            return {}
    ...
    # Alarms (with automatic expired temporary snooze cleanup)
                ...
                try:
                    with open(self.alarms_file, "r", encoding="utf-8") as f:
                        ...
                        if isinstance(data, list):
                            ...
                            has_pruned = False
                            for a in data:
                                if not isinstance(a, dict) or not a.get("time"):
                                    continue
                                # Prune expired temporary snooze alarms
                                aid = str(a.get("id", ""))
                                if aid.startswith("snooze_") or a.get("is_snooze", False):
                                    ...
                                    if expiry:
                                        ...
                                        # Snooze entry with no recurring days past its trigger time
                                        t_str = a.get("time", "")
                                        try:
                                            t_parts = [int(p) for p in t_str.split(":")]
                                            a_dt = now_dt.replace(hour=t_parts[0], minute=t_parts[1], second=0, microsecond=0)
                                            if (now_dt - a_dt).total_seconds() > 300:
                                                has_pruned = True
                                                continue
                                        except Exception:
                                            pass
                                cleaned.append(a)
                            if has_pruned:
                                safe_atomic_write_json(self.alarms_file, cleaned)
                            return cleaned
                except Exception as e:
                    print(f"Corrupted alarms.json recovered: {e}")
                    backup_corrupt_file(self.alarms_file)
            return []
                        ...
                        if isinstance(data, list):
                            ...
                            for t in data:
                                if not isinstance(t, dict) or not t.get("time"):
                                    continue
                                tid = str(t.get("id", ""))
                                if tid.startswith("snooze_") or t.get("is_snooze", False):
                                    expiry = t.get("expires_at")
                                    if expiry:
                                        try:
                                            if datetime.fromisoformat(expiry) < now_dt:
                                                has_pruned = True
                                ...
                                cleaned.append(t)
                            if has_pruned:
                                safe_atomic_write_json(self.timetable_file, cleaned)
                            return cleaned
                except Exception as e:
                    print(f"Corrupted timetable.json recovered: {e}")
                    backup_corrupt_file(self.timetable_file)
            return []
        ...
        with self._lock:
            if os.path.exists(self.clipboard_file):
                try:
                    ...
                    backup_corrupt_file(self.clipboard_file)
            return []
        ...
        with self._lock:
            if os.path.exists(self.fileshelf_file):
                try:
                    ...
                    backup_corrupt_file(self.fileshelf_file)
            return []
        ...
        with self._lock:
            if os.path.exists(self.custom_sounds_file):
                try:
                    ...
                    backup_corrupt_file(self.custom_sounds_file)
            return []
