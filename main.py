...
from PyQt6.QtCore import Qt, QTimer, pyqtSignal, QPropertyAnimation, QEasingCurve, QSize, QThread, pyqtProperty, QObject, QUrl, QFileSystemWatcher, QPoint, QPointF, QRect, QRectF, QEvent
...
class DynamicIsland(QWidget):
    ...
    def __init__(self, storage: StorageManager = None, settings: SettingsManager = None):
        ...
        # Dedicated Isolated Authoritative Alarm & Timetable Scheduler Worker Thread (Zero GUI blocking)
        ...
        # Screen Topology Change Observers
        app_inst = QApplication.instance()
        if app_inst:
            app_inst.screenAdded.connect(self.on_screen_topology_changed)
            app_inst.screenRemoved.connect(self.on_screen_topology_changed)
            app_inst.primaryScreenChanged.connect(self.on_screen_topology_changed)
        ...
        # Dedicated Window Flags Watchdog (Protects against any runtime drift of Tool / Always-on-top state)
        self.window_watchdog_timer = QTimer(self)
        self.window_watchdog_timer.setInterval(30000)  # Every 30 seconds
        self.window_watchdog_timer.timeout.connect(self.verify_and_repair_window_flags)
        self.window_watchdog_timer.start()
    ...
    def on_screen_topology_changed(self, *args):
    ...
    def set_taskbar_visibility(self, visible: bool):
        """Dynamically toggles whether the window has an active button on the Windows Taskbar."""
        try:
            hwnd = int(self.winId())
            GWL_EXSTYLE = -20
    ...
    def apply_island_window_flags(self):
        """Authoritatively enforces the complete set of window flags, attributes, and Win32 styles for the floating overlay."""
        full_flags = (
            Qt.WindowType.Tool |
            Qt.WindowType.FramelessWindowHint |
            Qt.WindowType.WindowStaysOnTopHint
        )
        if self.windowFlags() != full_flags:
            self.setWindowFlags(full_flags)
    ...
    def enforce_win32_tool_topmost(self):
        """Directly verifies and enforces Win32 WS_EX_TOOLWINDOW, WS_EX_TOPMOST, and HWND_TOPMOST."""
        if getattr(self, '_minimized_to_tray', False) or self.isMinimized():
            return
        try:
            hwnd = int(self.winId())
            GWL_EXSTYLE = -20
            ...
            # Ensure TOOLWINDOW and TOPMOST are present; ensure APPWINDOW is cleared
            new_style = (style | WS_EX_TOOLWINDOW | WS_EX_TOPMOST) & ~WS_EX_APPWINDOW
            if new_style != style:
                ctypes.windll.user32.SetWindowLongW(hwnd, GWL_EXSTYLE, new_style)

            # Firmly assert HWND_TOPMOST in Win32 Z-order band
            HWND_TOPMOST = -1
            SWP_FLAGS = 0x0001 | 0x0002 | 0x0010 | 0x0020  # SWP_NOSIZE | SWP_NOMOVE | SWP_NOACTIVATE | SWP_FRAMECHANGED
            ctypes.windll.user32.SetWindowPos(hwnd, HWND_TOPMOST, 0, 0, 0, 0, SWP_FLAGS)
        except Exception:
            pass

    def verify_and_repair_window_flags(self):
        """Defensive safety net: periodically verifies that the island maintains its Tool and WindowStaysOnTopHint state."""
        if getattr(self, '_minimized_to_tray', False) or self.isMinimized():
            return
        try:
            target_flags = (
            ...
            if self.windowFlags() != target_flags:
                print("[WINDOW WATCHDOG] Qt window flags drifted! Silently repairing...", flush=True)
                self.apply_island_window_flags()
            ...
            needs_repair = (
                not (style & WS_EX_TOOLWINDOW) or
                not (style & WS_EX_TOPMOST) or
                bool(style & WS_EX_APPWINDOW)
            )

            if needs_repair:
                print("[WINDOW WATCHDOG] Win32 styles drifted! Silently repairing...", flush=True)
                self.enforce_win32_tool_topmost()
            else:
                # Silently reinforce HWND_TOPMOST in Win32 z-order band
                HWND_TOPMOST = -1
    ...
    def showEvent(self, event):
        ...
        if not getattr(self, '_minimized_to_tray', False):
            self.enforce_win32_tool_topmost()

    def changeEvent(self, event):
        if event.type() == QEvent.Type.WindowStateChange:
            if not self.isMinimized() and getattr(self, '_minimized_to_tray', False):
                self.restore_from_tray()
            elif not self.isMinimized() and not getattr(self, '_minimized_to_tray', False):
                self.enforce_win32_tool_topmost()
        elif event.type() == QEvent.Type.ActivationChange:
            if not self.isMinimized() and not getattr(self, '_minimized_to_tray', False):
                self.enforce_win32_tool_topmost()
    ...
    def init_ui(self):
        ...
        self.install_drag_filter_recursive(self)
        self.install_drag_filter_recursive(self.container_frame)
        self.install_drag_filter_recursive(self.pill_widget)
        ...
        if not os.path.exists(logo_p):
            logo_p = os.path.join(os.path.dirname(__file__), "app_icon.png")
        ...
        # Smooth Geometry Animation (Snappy 140ms responsiveness)
        ...
        self.priority_notif_thread.notifications_updated.connect(self.tab_home.update_notifications)
    ...
    def apply_container_style(self):
        ...
        if hasattr(self, 'btn_min_to_tray'):
            min_bg = "rgba(255, 255, 255, 0.08)" if mode == "dark" else "rgba(0, 0, 0, 0.06)"
            min_border = "rgba(255, 255, 255, 0.16)" if mode == "dark" else "rgba(0, 0, 0, 0.14)"
            self.btn_min_to_tray.setStyleSheet(f"""
                QPushButton {{
                    background-color: {min_bg};
    ...
    def update_ambient_aura(self, data: dict = None):
        if not hasattr(self, 'container_frame') or not self.container_frame:
            return
        if data is None:
    ...
    def update_window_position(self, frame_w: int, frame_h: int):
        ...
        # Strict screen boundary clamping to prevent off-screen spawning across DPI/monitor switches
        min_x = geo.x()
    ...
    def enterEvent(self, event):
        ...
        if not self.is_expanded:
            if not self.is_alarm_alert_active and QApplication.activeModalWidget() is None:
                delay = max(50, int(self.settings.get("hover_delay_ms", 300)))
    ...
    def leaveEvent(self, event):
        ...
        # If collapsed and cursor moved outside window bounds, immediately cancel hover expansion
        if not self.is_expanded:
    ...
    def eventFilter(self, watched, event):
        ...
        if et == QEvent.Type.MouseButtonPress:
            if event.button() == Qt.MouseButton.RightButton:
                ...
                self._right_drag_start_cursor = QCursor.pos()
                self._right_drag_click_offset = self._right_drag_start_cursor - self.pos()
                self._right_drag_distance = 0
                self.leave_timer.stop()
                self.hover_timer.stop()
                self.mouse_inside_collapsed_since = None
            ...
            if getattr(self, '_right_drag_active', False):
                ...
                curr_cursor = QCursor.pos()
                if self._right_drag_start_cursor is None:
                    self._right_drag_start_cursor = curr_cursor
                if self._right_drag_click_offset is None:
                    self._right_drag_click_offset = curr_cursor - self.pos()

                delta = curr_cursor - self._right_drag_start_cursor
                self._right_drag_distance = delta.manhattanLength()
                target_pos = curr_cursor - self._right_drag_click_offset

                # Hardware-accelerated Win32 direct frame repositioning (0.0002ms, zero Qt event backlog)
                try:
                    hwnd = int(self.winId())
                    SWP_FLAGS = 0x0001 | 0x0004 | 0x0010  # SWP_NOSIZE | SWP_NOZORDER | SWP_NOACTIVATE
                    ctypes.windll.user32.SetWindowPos(hwnd, 0, target_pos.x(), target_pos.y(), 0, 0, SWP_FLAGS)
                except Exception:
            ...
            if getattr(self, '_right_drag_active', False) and event.button() == Qt.MouseButton.RightButton:
                ...
                if getattr(self, '_right_drag_distance', 0) > 4:
                    ...
                    self.show_island_context_menu(curr_cursor)
                    return True
    ...
    def _save_dragged_position(self, explicit_target_pos=None):
        ...
        try:
            ...
            if pos_setting in ("top_center", "bottom_center"):
                ...
                center_screen_x = geo.x() + (geo.width() / 2.0)
                offset_x = center_island_x - center_screen_x
            elif pos_setting == "top_left":
                offset_x = current_x - geo.x()
            elif pos_setting == "top_right":
                offset_x = (geo.x() + geo.width()) - (current_x + win_w)
            else:
                offset_x = 0

            if pos_setting == "bottom_center":
                offset_y = (geo.y() + geo.height()) - (current_y + win_h)
            else:
    ...
    def check_force_collapse_safety(self):
        ...
        if not self.is_expanded:
            ...
            if self.frameGeometry().contains(cursor_pos):
                delay = max(50, int(self.settings.get("hover_delay_ms", 300)))
                if not self.hover_timer.isActive() and not self.is_expanded:
                    self.hover_timer.start(delay)
            else:
                self.hover_timer.stop()
                self.mouse_inside_collapsed_since = None
    ...
    def install_drag_filter_recursive(self, parent_widget: QWidget):
        """Recursively installs right-click drag event filters across all child widgets for 100% reliable drag responsiveness."""
    ...
    def toggle_expanded_state(self):
        if getattr(self, '_minimized_to_tray', False) or self.isMinimized() or self.isHidden():
            self.restore_from_tray()
    ...
    def restore_from_tray(self):
        ...
        if hasattr(self, '_last_restore_pos') and self._last_restore_pos:
            self.move(self._last_restore_pos)
        else:
            self.update_window_position(curr_w, curr_h)
    ...
    def init_system_tray(self):
        ...
        act_restore = QAction("✨ Restore Dynamic Island", self)
        act_restore.triggered.connect(self.restore_from_tray)

        act_min = QAction("🗕 Minimize to System Tray", self)
        act_min.triggered.connect(self.minimize_to_tray)

        act_toggle = QAction("🏝️ Toggle Expanded State", self)
        ...
        act_home.triggered.connect(lambda: (self.restore_from_tray(), self.expand_island(), self.switch_tab(0)))
    ...
    def open_full_app_window(self, view_index: int = None):
        ...
        if not self.full_app_window:
            ...
            self.full_app_window = DlivesFullAppWindow(self.storage, self.settings)
    ...
    def on_tray_activated(self, reason):
        if reason in (QSystemTrayIcon.ActivationReason.Trigger, QSystemTrayIcon.ActivationReason.DoubleClick):
            if self.isHidden() or getattr(self, '_minimized_to_tray', False) or self.isMinimized():
                self.restore_from_tray()
            else:
    ...
    def on_fast_clock_tick(self):
        ...
        # Periodic Defensive Watchdog Check (every ~30s / 150 ticks of 200ms clock)
        if not hasattr(self, '_watchdog_fast_tick'):
            self._watchdog_fast_tick = 0
        self._watchdog_fast_tick += 1
        if self._watchdog_fast_tick >= 150:
            self._watchdog_fast_tick = 0
            self.verify_and_repair_window_flags()

        # Fast 200ms Audio Mute & Volume Live Sync (Instant response to keyboard/mixer mute)
        if not hasattr(self, '_fast_mute_tick'):
            self._fast_mute_tick = 0
        self._fast_mute_tick += 1
        if self._fast_mute_tick % 2 == 0:
            try:
                is_muted = SystemMonitor.get_master_mute()
                vol = SystemMonitor.get_master_volume()
                self.pill_widget.update_volume_and_mute(vol, is_muted or (vol <= 0))
            except Exception:
                pass
    ...
    def is_pomodoro_focus_active(self) -> bool:
        """Returns True if a Pomodoro Focus session is currently active and counting down."""
        try:
            if hasattr(self, 'tab_home') and hasattr(self.tab_home, 'card_pomo') and self.tab_home.card_pomo:
                return bool(self.tab_home.card_pomo.is_running and self.tab_home.card_pomo.current_mode == "focus")
        except Exception:
            pass
        return False
    ...
    def trigger_alarm_alert(self, title: str, is_timetable: bool = False, time_str: str = "", days: list = None, task_id: str = "", sound_path: str = None):
        # Pomodoro Focus DND Rules Check
        if self.is_pomodoro_focus_active():
            if is_timetable and not self.settings.get("pomodoro_allow_timetable", True):
                print(f"[Pomodoro DND]: Silenced Timetable task '{title}' due to active Focus session rule.")
                return
            elif not is_timetable and not self.settings.get("pomodoro_allow_alarms", True):
                print(f"[Pomodoro DND]: Silenced Alarm '{title}' due to active Focus session rule.")
                return
    ...
    def _collapse_after_modal_dismiss(self):
        """Immediately and snappily collapses island after an alert/notification modal closes with lightning-fast 110ms animation."""
        self.is_expanded = False
        self.dashboard_widget.hide()
        self.alarm_banner_card.hide()
        if hasattr(self, 'notification_banner_card'):
    ...
    def snooze_alarm_alert(self, alarm_data: dict, minutes: int):
        ...
        expires_at_str = (now + timedelta(minutes=minutes + 15)).isoformat()
    ...
    def trigger_notification_popup(self, notif_data: dict):
        ...
        # Pomodoro Focus Do-Not-Disturb Check
        if self.is_pomodoro_focus_active() and self.settings.get("pomodoro_dnd_notifications", True):
            print(f"[Pomodoro DND]: Silenced popup notification for '{notif_data.get('title')}' during active Focus session.", flush=True)
            return
...
def main():
    try:
        ctypes.windll.shell32.SetCurrentProcessExplicitAppUserModelID("dlives.desktop.dynamicisland.1.0")
    except Exception:
        pass
    ...
    if os.path.exists(icon_path):
        island.setWindowIcon(QIcon(icon_path))
