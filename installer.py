...
def create_shortcut(target_path, shortcut_path, icon_path=None, description=""):
    try:
        ...
        try:
            ...
            w_dir = str(os.path.dirname(target_path)).replace("'", "''")
            i_path = str(ico).replace("'", "''")
            desc = str(description).replace("'", "''")
            ps_cmd = (
                f"$ws = New-Object -ComObject WScript.Shell; "
                f"$s = $ws.CreateShortcut('{s_path}'); "
...
def register_uninstaller(install_dir, exe_path):
    try:
        ...
        with winreg.CreateKey(winreg.HKEY_CURRENT_USER, key_path) as key:
            ...
            winreg.SetValueEx(key, "Publisher", 0, winreg.REG_SZ, "Fugentech")
            ...
            # Add EstimatedSize in KB (~65MB = 66560KB)
            winreg.SetValueEx(key, "EstimatedSize", 0, winreg.REG_DWORD, 66560)
    except Exception as e:
...
class InstallWorker(QThread):
    ...
    def run(self):
        try:
            ...
            self.progress.emit(30, "Extracting and copying application binaries...")
            ...
            possible_exe_sources = [
                ...
                os.path.join(bundle_dir, "SanLives.exe"),
            ...
            # Dynamic scan of bundle_dir for any binary payload
            if os.path.exists(bundle_dir):
                for root, _, files in os.walk(bundle_dir):
                    for fname in files:
                        fpath = os.path.join(root, fname)
                        fl = fname.lower()
                        if fl in ("dlives_payload.bin", "dlives.exe", "sanlives.exe", "payload.bin"):
                            if fpath not in possible_exe_sources:
                                possible_exe_sources.insert(0, fpath)
                        elif fl.endswith(".exe") or fl.endswith(".bin"):
                            try:
                                if os.path.getsize(fpath) > 10 * 1024 * 1024:  # > 10 MB payload
                                    if fpath not in possible_exe_sources:
                                        possible_exe_sources.append(fpath)
                            except Exception:
                                pass
            ...
            for src_p in possible_exe_sources:
                if os.path.exists(src_p) and os.path.isfile(src_p):
                    try:
                        ...
                        print(f"[INSTALLER WARN]: Failed copying '{src_p}': {copy_err}")
...
if exist "%USERPROFILE%\\Desktop\\Dlives.lnk" del /f /q "%USERPROFILE%\\Desktop\\Dlives.lnk" >nul 2>&1
if exist "%USERPROFILE%\\OneDrive\\Desktop\\Dlives.lnk" del /f /q "%USERPROFILE%\\OneDrive\\Desktop\\Dlives.lnk" >nul 2>&1
if exist "%APPDATA%\\Microsoft\\Windows\\Start Menu\\Programs\\Dlives.lnk" del /f /q "%APPDATA%\\Microsoft\\Windows\\Start Menu\\Programs\\Dlives.lnk" >nul 2>&1
