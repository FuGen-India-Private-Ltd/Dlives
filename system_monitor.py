...
def apply_win32_acrylic(hwnd: int, gradient_color_hex: str = "#000000", opacity: float = 0.88, is_dark: bool = True) -> bool:
    """
    ...
    try:
        ...
        if len(h) == 6:
            ...
            r, g, b = (0, 0, 0) if is_dark else (240, 244, 250)
...
WINRT_EXECUTOR = concurrent.futures.ThreadPoolExecutor(max_workers=2, thread_name_prefix="WinRTWorker")
...
def _run_winrt_in_worker_thread(async_func, timeout_sec=2.0):
    ...
    try:
        future = WINRT_EXECUTOR.submit(_worker)
    ...
    def get_windows_notifications() -> dict:
        ...
        async def _async_fetch():
            try:
                ...
                # Auto-delete / auto-prune notifications for the day (only capture today's notifications)
                ...
                for n in list(notifs):
                    try:
                        if hasattr(n, "creation_time") and n.creation_time:
                            ...
                            # Discard any notifications older than today (auto-delete for previous days)
    ...
    def run(self):
        while self.running:
            try:
                ...
                volume = SystemMonitor.get_master_volume()
                is_muted = SystemMonitor.get_master_mute() or (volume <= 0)
                brightness = SystemMonitor.get_brightness()
                cpu = SystemMonitor.get_cpu_usage()
    ...
    def run(self):
        try:
            ...
            while self.running:
                try:
                    res = SystemMonitor.get_windows_notifications()
                    items = res.get("notifications", []) if isinstance(res, dict) else []
                    self.notifications_updated.emit(items)
